defmodule NervesWifibroadcast.Examples.WfbIngressSmoke do
  ...
  defp print_started_banner(opts) do
    ...
    IO.puts("[wfb_ingress_smoke] started")
    IO.puts("[wfb_ingress_smoke] interfaces=#{Enum.join(Keyword.fetch!(opts, :interfaces), ",")}")
...
defmodule NervesWifibroadcast.Examples.WfbIngressSmoke.Pipeline do
  ...
  def handle_call({:set_radio_ports, radio_ports}, _ctx, state) do
    ...
    if unknown == [] do
      actions = [notify_child: {:ingress, {:set_radio_ports, radio_ports}}, reply: :ok]
      {actions, state}
    else
  ...
  def handle_child_notification(notification, child, _ctx, state) do
    IO.puts("[wfb_ingress_smoke] #{inspect(child)} notification: #{inspect(notification)}")
    {[], state}
  end

  @impl true
  def handle_child_playing(child, _ctx, state) do
    IO.puts("[wfb_ingress_smoke] #{inspect(child)} is playing")
    {[], state}
  end
end
...
defmodule NervesWifibroadcast.Examples.WfbIngressSmoke.ChannelSink do
  ...
  def handle_init(_ctx, opts) do
    ...
    state = %{
      ...
      last_summary_data_packets: 0,
      last_summary_packets: 0,
      last_summary_session_packets: 0,
  ...
  def handle_start_of_stream(:input, _ctx, state) do
    IO.puts("[wfb_ingress_smoke #{scope_label(state)}] stream started")
    {[], state}
  end
  ...
  def handle_stream_format(:input, %StreamFormat{} = format, _ctx, state) do
    IO.puts(
      "[wfb_ingress_smoke #{scope_label(state)}] stream format channel_id=#{format_channel_id(format.channel_id)} link=#{format_link_id(format.link_id)} port=#{format.radio_port} encrypted=#{format.encrypted?}"
    )
  ...
  def handle_buffer(:input, buffer, _ctx, state) do
    ...
    next_state = %{
      state
      | data_packets: state.data_packets + if(wfb.packet_type == :data, do: 1, else: 0),
        last_radiotap: radiotap,
        last_wfb: wfb,
        packets: state.packets + 1,
        payload_bytes: state.payload_bytes + byte_size(buffer.payload),
        session_packets: state.session_packets + if(wfb.packet_type == :session, do: 1, else: 0)
    }

    {[], maybe_print_preview(next_state, buffer)}
  end
  ...
  def handle_tick(@summary_timer, _ctx, state) do
    ...
    delta_data_packets = state.data_packets - state.last_summary_data_packets
    delta_session_packets = state.session_packets - state.last_summary_session_packets
    ...
    IO.puts(
      "[wfb_ingress_smoke #{scope_label(state)}] summary packets=#{delta_packets} data=#{delta_data_packets} session=#{delta_session_packets} rate=#{format_float(packets_per_second)} pkt/s bytes=#{delta_bytes} rate=#{format_byte_rate(bytes_per_second)} bitrate=#{format_bit_rate(bits_per_second)} #{format_wfb(state.last_wfb)} #{format_radiotap(state.last_radiotap)}"
    )
    ...
    next_state = %{
      ...
      | last_summary_at_ms: now,
        ...
        last_summary_data_packets: state.data_packets,
        last_summary_packets: state.packets,
        last_summary_session_packets: state.session_packets
    }
  ...
  def handle_end_of_stream(:input, _ctx, state) do
    IO.puts("[wfb_ingress_smoke #{scope_label(state)}] end of stream")
    {[], state}
  end
  ...
  defp maybe_print_preview(state, buffer) do
    ...
    IO.puts(
      "[wfb_ingress_smoke #{scope_label(state)}] packet=#{state.packets} type=#{wfb.packet_type} payload_bytes=#{byte_size(buffer.payload)} #{format_wfb(wfb)} #{format_radiotap(get_in(buffer.metadata, [:radio, :radiotap]))} preview=#{hex_preview(buffer.payload, state.max_preview_bytes)}"
    )

    %{state | preview_left: state.preview_left - 1}
  end

  defp format_wfb(nil), do: "wfb=unavailable"

  defp format_wfb(%{packet_type: :data, block_idx: block_idx, fragment_idx: fragment_idx}) do
    "block=#{block_idx} frag=#{fragment_idx}"
  end

  defp format_wfb(%{packet_type: :session, session_nonce: session_nonce}) do
    nonce_preview =
      session_nonce
      |> binary_part(0, min(byte_size(session_nonce), 8))
      |> Base.encode16(case: :lower)

    "session_nonce=#{nonce_preview}"
  end
