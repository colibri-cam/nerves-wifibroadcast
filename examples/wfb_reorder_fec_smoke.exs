defmodule NervesWifibroadcast.Examples.WfbReorderFecSmoke do
  ...
  defp normalize_opts(opts) do
    ...
    cond do
      ...
      not is_integer(ring_size) or ring_size < 1 ->
        raise ArgumentError, "expected :ring_size to be a positive integer"
...
defmodule NervesWifibroadcast.Examples.WfbReorderFecSmoke.ChannelSink do
  ...
  def handle_init(_ctx, opts) do
    ...
    state = %{
      ...
      fec_only_shards: 0,
      last_radiotap: nil,
      last_session_format: nil,
      last_shard: nil,
      last_summary_at_ms: now,
      last_summary_bytes: 0,
      last_summary_data_shards: 0,
      last_summary_fec_only_shards: 0,
      last_summary_recovered_shards: 0,
      last_summary_total_shards: 0,
    ...
    {[],
     %{
       ...
       | link_id: format.link_id,
         ...
         last_session_format: format
     }}
  end

  @impl true
  def handle_buffer(:input, buffer, _ctx, state) do
    shard = parse_shard(buffer)
    radiotap = get_in(buffer.metadata, [:radio, :radiotap])

    next_state = %{
      state
      | data_shards: state.data_shards + if(shard.kind == :data, do: 1, else: 0),
        fec_only_shards: state.fec_only_shards + if(shard.kind == :fec_only, do: 1, else: 0),
        last_radiotap: radiotap,
        last_shard: shard,
        payload_bytes: state.payload_bytes + byte_size(shard.payload),
        recovered_shards: state.recovered_shards + if(shard.recovered?, do: 1, else: 0),
        total_shards: state.total_shards + 1
    }

    {[], maybe_print_preview(next_state, shard)}
  end
  ...
  def handle_tick(@summary_timer, _ctx, state) do
    ...
    delta_total_shards = state.total_shards - state.last_summary_total_shards
    delta_data_shards = state.data_shards - state.last_summary_data_shards
    delta_fec_only_shards = state.fec_only_shards - state.last_summary_fec_only_shards
    delta_recovered_shards = state.recovered_shards - state.last_summary_recovered_shards
    delta_bytes = state.payload_bytes - state.last_summary_bytes
    shards_per_second = delta_total_shards * 1_000 / elapsed_ms
    ...
    next_state = %{
      ...
      | last_summary_at_ms: now,
        ...
        last_summary_data_shards: state.data_shards,
        last_summary_fec_only_shards: state.fec_only_shards,
        last_summary_recovered_shards: state.recovered_shards,
        last_summary_total_shards: state.total_shards
    }
  ...
  defp maybe_print_preview(%{preview_left: 0} = state, _shard), do: state
  ...
  defp parse_shard(buffer) do
    wfb = buffer.metadata.wfb

    case buffer.payload do
      <<flags, packet_size::big-16, rest::binary>> ->
        payload = binary_part(rest, 0, min(packet_size, byte_size(rest)))

        %{
          block_idx: wfb.block_idx,
          emission: wfb.emission,
          flags: flags,
          fragment_idx: wfb.fragment_idx,
          kind: if((flags &&& @fec_only_flag) != 0, do: :fec_only, else: :data),
          ordered_seq: wfb.ordered_seq,
          packet_size: packet_size,
          payload: payload,
          recovered?: Map.has_key?(buffer.metadata, :recovery),
          receiver_mask: get_in(buffer.metadata, [:recovery, :receiver_mask]),
          truncated?: byte_size(rest) < packet_size
        }
  ...
  defp format_session(%OrderedShardStreamFormat{} = format) do
    "session_epoch=#{format.epoch} fec=#{format.fec_type}:#{format.fec_k}/#{format.fec_n}"
  end

  defp format_shard(nil), do: "shard=unavailable"

  defp format_shard(shard) do
    parts = [
      "block=#{shard.block_idx}",
      "frag=#{shard.fragment_idx}",
      "seq=#{shard.ordered_seq}",
      "kind=#{shard.kind}",
      "emission=#{shard.emission}"
    ]

    parts =
      if is_integer(shard.flags) and is_integer(shard.packet_size) do
        parts ++
          [
            "flags=0x" <> String.pad_leading(Integer.to_string(shard.flags, 16), 2, "0"),
            "packet_size=#{shard.packet_size}"
          ]
      else
        parts
      end
    ...
    parts =
      if shard.recovered? do
        parts ++
          ["recovered", "receiver_mask=0x" <> Integer.to_string(shard.receiver_mask || 0, 16)]
      else
        parts
      end

    parts = if shard.truncated?, do: parts ++ ["truncated"], else: parts
    Enum.join(parts, " ")
  end
